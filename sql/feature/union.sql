...
#
# HEAP
# 有极高的插入、更新和查询效率，默认使用哈希索引 只主流在内存里的临时表格，保存的数据具有不稳定性
#适用-------------
# 用于临时表
#
# INNODB
# 有提交、回滚和崩溃恢复能力的失误安全存储引擎 读取速度比ISAM慢
#适用-------------
# 表更新和查询都相当频繁，且表锁定的计划比较大的情况
#
# ARCHIVE
# 有高效的插入速度 不支持索引，仅支持insert和select语句
#适用-------------
...
#修改数据表引擎
# alter table Buyu.user
#   engine = InnoDB;
# alter table Buyu.user_stat
#   engine = InnoDB;
