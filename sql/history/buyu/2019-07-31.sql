drop table if exists z_huizong_qudao_hour;
-- auto-generated definition
create table z_huizong_qudao_hour
(
    tm             datetime         not null,
    flavor         varchar(50)      not null comment '渠道来源',
    ...
    coin_pay       bigint default 0 null comment '距离上一次统计时间充值增加金币',
    coin_day       bigint default 0 null comment '距离上一次统计时间签到增加金币',
    coin_vip       bigint default 0 null comment '距离上一次统计时间vip奖励增加金币',
    coin_card      bigint default 0 null comment '距离上一次统计时间周月卡每日奖励增加金币',
    coin_rank      bigint default 0 null comment '距离上一次统计时间排行榜奖励增加金币',
    coin_alms      bigint default 0 null comment '距离上一次统计时间免费破产增加金币',
    coin_task      bigint default 0 null comment '距离上一次统计时间成长任务增加金币',
    coin_mission   bigint default 0 null comment '距离上一次统计时间在线任务,每日任务增加金币',
    coin_mail      bigint default 0 null comment '距离上一次统计时间邮件奖励增加金币',
    coin_key       bigint default 0 null comment '距离上一次统计时间CDKey增加金币',
    coin_invite    bigint default 0 null comment '距离上一次统计时间邀请奖励增加金币',
    coin_lzc       bigint default 0 null comment '距离上一次统计时间龙珠场抽宝箱增加金币',
    coin_yt        bigint default 0 null comment '距离上一次统计时间鱼货提取/偷取增加金币',
    coin_fanli     bigint default 0 null comment '距离上一次统计时间返利增加金币(含充值，售卖)',
    coin_other     bigint default 0 null comment '距离上一次统计时间活动奖励76，解锁炮值(目前无需解锁)增加金币',
    bill_room2     bigint default 0 null comment '距离上一次统计时间初级场当前流水',
    bill_room3     bigint default 0 null comment '距离上一次统计时间中级场当前流水',
    bill_room8     bigint default 0 null comment '距离上一次统计时间红包场当前流水',
    coin_fee_room2 bigint default 0 null comment '距离上一次统计时间初级场预估消耗金币',
    coin_fee_room3 bigint default 0 null comment '距离上一次统计时间中级场预估消耗金币',
    coin_fee_room8 bigint default 0 null comment '距离上一次统计时间红包场预估消耗金币',
    coin_fee_fort  bigint default 0 null comment '距离上一次统计时间购买炮台消耗金币',
    coin_in        bigint default 0 comment '距离上一次统计时间接收龙珠',
    coin_out       bigint default 0 comment '距离上一次统计时间赠送龙珠',
...
create
    definer = root@`192.168.100.3` procedure proc_huizong_by_flavor(IN vFlavor text)
exec:
BEGIN
    ...
    declare vTGSendCoin1 bigint;
    declare vTGSendCoin2 bigint;
...
#推广员赠送金币
    select ifnull(sum(coin), 0) into vTGSendCoin1
    from agent_coin_record
             inner join user a on agent_coin_record.uid = a.uid
             inner join user b on agent_coin_record.buyerId = b.uid
    where createTime >= vTmBegin
      and createTime < vTmEnd;

    select ifnull(sum(case transfer_type
                          when 1 then transfer_count * 10000
                          when 2 then transfer_count * 100000
                          when 3 then transfer_count * 1000000
                          else 0 end), 0) into vTGSendCoin2
    from dragon_transfer_log
             inner join user a on dragon_transfer_log.sender_uid = a.uid
             inner join user b on dragon_transfer_log.recv_uid = b.uid
