...
create procedure agent_promotion_reward()
  BEGIN
    ...
    select date_format(DATE_ADD(sysdate(), INTERVAL -1 DAY), '%Y-%m-%d')
    into v_createTime;
    ...
    myLoop: loop
      ...
      into v_num
      from user
      where bind_uid = v_uid and DATEDIFF(bind_date, NOW()) = -1;
    ...
    insert into day_coinSummary_log (uid, stats_date, coin_before, coin_after, coin_change, fee, change_type, game_type, room_id, round, oid, type)
      (
        ...
        select
          ...
          a.oid,
          b.type
        from coin_log a, user b
        where
          a.round > 0 and a.change_type = 3 and a.uid = b.uid and add_time >= ADDDATE(CURDATE(), INTERVAL -1 DAY) AND
          add_time < CURDATE()
      );
  END//
DELIMITER ;
      ...
      (
        ...
        from (
               ...
               from (
                      ...
                      where u.type = 2 and u2.type != 2 and DATEDIFF(addTime, NOW()) = -1) a
               ...
               from (
                      ...
                      where u.type = 0 and s.type = 2 and DATEDIFF(addTime, NOW()) = -1) a
...
create procedure day_pool_log()
  BEGIN
    ...
    insert into day_pool_log (stats_date, room_id, fee)
      ...
      (select
         date_sub(curdate(), interval 1 day) day,
         a.room_id,
         (a.fee - b.fee)                     fee
       from
         (select
...
create procedure day_summary_log()
  BEGIN
    ...
    select ifnull(count(distinct uid), 0)
    into v_rechargCount
    from order_log
    where result in (0, 1) and issandbox = 0 and (transtime >= v_stats_date and transtime < v_stats_date1);


    select ifnull(sum(coin_change), 0)
    into v_rechargeCoin
    from coin_log
    where change_type in (7, 8, 9, 15) and (transtime >= v_stats_date and transtime < v_stats_date1);
    ...
    into v_reward
    from coin_log
    where change_type in (0, 1, 11, 12, 13, 16, 17, 19, 20) and add_time >= v_stats_date and add_time < v_stats_date1;
...
create procedure day_summary_log_new()
  BEGIN
    ...
    on duplicate key update flowing = v_flowing, room1 = v_room1, room2 = v_room2, room3 = v_room3, room4 = v_room4,
      ...
      newbillave                    = v_newBillAve, totalbillave = v_totalBillAve;
...
create procedure everyday_analysis()
  BEGIN
    ...
    then
      ...
      set statsDate = v_statsDate, statsHour = v_statsHour, win1 = v_win1, win2 = v_win2, win3 = v_win3
...
create procedure stat_remain_player()
  BEGIN
    ...
    insert into stats_remain (dru, stat_time, add_time) select
                                                          count(uid),
    ...
    set second_day = (
      ...
      select (
        ...
        (select count(distinct u.uid)
         from login_log l left join user u on u.uid = l.uid and (u.reg_tm between days_ago_2 and yesterday) and
    ...
    set third_day = (
      ...
      select (
        ...
        (select count(distinct u.uid)
         from login_log l left join user u on u.uid = l.uid and (u.reg_tm between days_ago_3 and days_ago_2) and
                                              (login_time between yesterday and today))
    ...
    set fourth_day = (
      ...
      select (
        ...
        (select count(distinct u.uid)
         from login_log l left join user u on u.uid = l.uid and (u.reg_tm between days_ago_4 and days_ago_3) and
                                              (login_time between yesterday and today))
    ...
    set seventh_day = (
      ...
      select (
        ...
        (select count(distinct u.uid)
         from login_log l left join user u on u.uid = l.uid and (u.reg_tm between days_ago_7 and days_ago_6) and
                                              (login_time between yesterday and today))
    ...
    set eighth_day = (
      ...
      select (
        ...
        (select count(distinct u.uid)
         from login_log l left join user u on u.uid = l.uid and (u.reg_tm between days_ago_8 and days_ago_7) and
                                              (login_time between yesterday and today))
    ...
    SET fourteen_day = (
      select (
        (select count(distinct u.uid)
         from login_log l left join user u on u.uid = l.uid and (u.reg_tm between days_ago_14 and days_ago_13) and
                                              (login_time between yesterday and today))
        /
        (select count(uid)
         from user
    ...
    SET fifteenth_day = (
      select (
        (select count(distinct u.uid)
         from login_log l left join user u on u.uid = l.uid and (u.reg_tm between days_ago_15 and days_ago_14) and
                                              (login_time between yesterday and today))
        /
        (select count(uid)
         from user
    ...
    SET thirtieth_day = (
      select (
        (select count(distinct u.uid)
         from login_log l left join user u on u.uid = l.uid and (u.reg_tm between days_ago_30 and days_ago_29) and
                                              (login_time between yesterday and today))
        /
        (select count(uid)
         from user
    ...
    SET thirtieth_first_day = (
      select (
        (select count(distinct u.uid)
         from login_log l left join user u on u.uid = l.uid and (u.reg_tm between days_ago_31 and days_ago_30) and
                                              (login_time between yesterday and today))
        /
        (select count(uid)
         from user
...
create procedure stats_coin()
  BEGIN
    ...
    into v_total_coin
    from user_stat s left join user u on u.uid = s.uid
    where u.type != 3;

    select ifnull(sum(s.pearl), 0)
    into v_pearl1
    from user_stat s left join user u on u.uid = s.uid
    where u.type != 3;

    select ifnull(sum(s.db1), 0)
    into v_db1
    from user_stat s left join user u on u.uid = s.uid
    where u.type != 3;

    select ifnull(sum(s.db2), 0)
    into v_db2
    from user_stat s left join user u on u.uid = s.uid
    where u.type != 3;

    select ifnull(sum(s.db3), 0)
    into v_db3
    from user_stat s left join user u on u.uid = s.uid
    where u.type != 3;

    select ifnull(sum(a + b + c + d), 0)
    into v_dbcoin
    from (select
            sum(pearl * 100)   a,
            sum(db1 * 10000)   b,
    ...
    on duplicate key update total_coin = v_total_coin, pearl = v_pearl1, db1 = v_db1, db2 = v_db2, db3 = v_db3,
      dbcoin                           = v_dbcoin, yulecoin = v_yulecoin, stock_large = v_stock_large,
      stock_normal                     = v_stock_normal, bibei_large = v_bibei_large, bibei_normal = v_bibei_normal,

      fee_large                        = v_fee_large, fee_normal = v_fee_normal, buyufee = v_buyufee,
      recharge_coin                    = v_rechargeCoin, reward_coin = v_rewardCoin, pool1 = v_pool1, pool2 = v_pool2,
      pool3                            = v_pool3, pool4 = v_pool4,
      win1                             = v_win1, pool1 = v_pool1, fee1 = v_fee1, dragonball1 = v_dragonball1,
      win2                             = v_win2, pool2 = v_pool2, fee2 = v_fee2, dragonball2 = v_dragonball2,
      win3                             = v_win3, pool3 = v_pool3, fee3 = v_fee3, dragonball3 = v_dragonball3,
...
create procedure stats_summary()
  BEGIN
    ...
    from online_count
    where tms = 5 and tp = 1 and (addtime >= v_stats_date and addtime < v_stats_date1);
    ...
    on duplicate key update reg_user_count = v_reg_user_count, login_user_count = v_login_user_count,
      max_online                           = v_max_online, avg_online = v_avg_online,
      deposit_user_count                   = v_deposit_user_count, depost_count = v_depost_count,
      pay_money                            = v_pay_money, total_coin = v_total_coin, total_exp = v_total_exp;
...
create procedure stats_user_coin()
  BEGIN
    -- 用户流水
    insert into stats_user_coin (stats_date, uid, coin) select
                                                          now(),
                                                          uid,
                                                          sum(coin_change)
                                                        from coin_log
...
CREATE PROCEDURE proc_get_today_flag(in uidIn       bigint, in uuidIn text, in tmIn DATETIME, in check_week int,
  ...
  BEGIN
    ...
    into day_uuid
    from newer7_log
    where uuid = uuidIn AND tm >= tmIn;
    #检查每日奖励领取 -uid
    select count(1)
    into day_uid
    from newer7_log
    where uid = uidIn AND tm >= tmIn;
    ...
    then
      ...
      into week
      from awards_log
      where awards_type = 45 and uid = uidIn AND tm >= tmIn;
    end if;
    ...
    then
      ...
      into month
      from awards_log
      where awards_type = 47 and uid = uidIn AND tm >= tmIn;
    end if;
    ...
    then
      ...
      into vip
      from awards_log
      where awards_type = 29 and uid = uidIn AND tm >= tmIn;
    end if;
